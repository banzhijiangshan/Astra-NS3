             ...
             uint32_t mpi_enabled)
{
    ...
    for (uint32_t i = node_num; i < All_num; i++)
    {
        for (uint32_t j = 0; j < 8; j++)
        {
            ...
            nbr2if[n.Get(index)][n.Get(i)].up = true;
            nbr2if[n.Get(index)][n.Get(i)].delay = 0;
            nbr2if[n.Get(index)][n.Get(i)].bw =
                DynamicCast<NVLinkNetDevice>(d.Get(0))->GetDataRate().GetBitRate();
            nbr2if[n.Get(i)][n.Get(index)].idx =
                DynamicCast<NVLinkNetDevice>(d.Get(1))->GetIfIndex();
            nbr2if[n.Get(i)][n.Get(index)].up = true;
            nbr2if[n.Get(i)][n.Get(index)].delay = 0;
            nbr2if[n.Get(i)][n.Get(index)].bw =
                DynamicCast<NVLinkNetDevice>(d.Get(1))->GetDataRate().GetBitRate();
        }
    }
    //////////////////
