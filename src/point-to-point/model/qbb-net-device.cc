    ...
    if (m_node->GetNodeType() == 0)
    {
        ...
        if (qIndex != -1024)
        {
            ...
            { // high prio
                ...
                m_traceDequeue(p, 0);
                // Added by myself
                int sender_node = m_node->GetId();
                int receiver_node = m_channel->GetQbbDevice(0)->GetNode()->GetId() == sender_node
                                        ? m_channel->GetQbbDevice(1)->GetNode()->GetId()
                                        : m_channel->GetQbbDevice(0)->GetNode()->GetId();
                int src_group = sender_node / 8;
                int dst_group = receiver_node / 8;
                if (src_group == dst_group)
                {
                    TransmitStartWithParticipants(p);
                }
                else
                    //////////////////
                    TransmitStart(p);
            ...
            // Added by myself
            int sender_node = lastQp->GetSrc();
            int receiver_node = lastQp->GetDest();
            int src_group = sender_node / 8;
            int dst_group = receiver_node / 8;
            if (src_group == dst_group)
            {
    ...
    {                                     // switch, doesn't care about qcn, just send
        ...
        if (p)
        {
            ...
            int receiver_node = m_channel->GetQbbDevice(0)->GetNode()->GetId() == sender_node
                                    ? m_channel->GetQbbDevice(1)->GetNode()->GetId()
                                    : m_channel->GetQbbDevice(0)->GetNode()->GetId();
            Ptr<Node> receiver = m_channel->GetQbbDevice(0)->GetNode()->GetId() == sender_node
                                     ? m_channel->GetQbbDevice(1)->GetNode()
                                     : m_channel->GetQbbDevice(0)->GetNode();
            int src_group = sender_node / 8;
            int dst_group = receiver_node / 8;
            if (src_group == dst_group && m_node->GetNodeType() == 0 &&
                receiver->GetNodeType() == 0)
            {
                TransmitStartWithParticipants(p);
            }
            else
...
// Added by myself
bool
QbbNetDevice::TransmitStartWithParticipants(Ptr<Packet> p)
{
    NS_LOG_FUNCTION(this << p);
    NS_LOG_LOGIC("UID is " << p->GetUid() << ")");
    ...
    Time txCompleteTime = txTime + m_tInterframeGap;
    NS_LOG_LOGIC("Schedule TransmitCompleteEvent in " << txCompleteTime.GetSeconds() << "sec");
    Simulator::Schedule(Time(0), &QbbNetDevice::TransmitComplete, this);

    bool result = m_channel->TransmitStart(p, this, Time(0));
    if (result == false)
    {
        m_phyTxDropTrace(p);
    }
