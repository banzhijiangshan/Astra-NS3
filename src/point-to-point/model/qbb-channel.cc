...
QbbChannel::TransmitStart(Ptr<Packet> p, Ptr<QbbNetDevice> src, Time txTime)
{
    ...
    // Added by myself
    // Assume NVlink is so fast that we can ignore the time delay currently.
    int src_id = m_link[wire].m_src->GetNode()->GetId();
    int dst_id = m_link[wire].m_dst->GetNode()->GetId();
    int src_group = src_id / 8;
    int dst_group = dst_id / 8;
